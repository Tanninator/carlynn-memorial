...
- **`ADMIN_TOKEN`** (Worker secret) — Bearer token for `/admin`, used by both `/api/rsvps` and `DELETE /api/lanterns/:id`. Cached client-side under `carlynn-memorial::admin-token`.
...
admin.html  token-gated panel for RSVPs and Lanterns; tabs share the same Bearer token
